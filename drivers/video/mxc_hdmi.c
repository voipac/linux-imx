...
static const struct fb_videomode wvga_mode = {
	/* 800x480 @ 60 Hz, 29.7 kHz hsync */
	NULL, 60, 800, 480, 30610, 96, 24, 10, 3, 72, 7,
...
#if defined (CONFIG_MXC_HDMI_FORCE_DVI_MODELIST)
static void mxc_hdmi_force_dvi_modelist(struct mxc_hdmi *hdmi)
{
	dev_dbg(&hdmi->pdev->dev, "%s\n", __func__);

	hdmi->edid_cfg.hdmi_cap = false;

	dev_info(&hdmi->pdev->dev, "Force default modelist\n");
...
#if defined (CONFIG_MXC_HDMI_FORCE_DVI_MODELIST)
	mxc_hdmi_force_dvi_modelist(hdmi);
#endif
	/* Setting video mode */
